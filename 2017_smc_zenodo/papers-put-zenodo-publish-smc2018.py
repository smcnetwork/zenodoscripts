...
dir_data = "_DATA_"
ACCESS_TOKEN = "GUudIg6kR1ETIdBVQcAT9U68nebJhPUfjdGR2RqGLtLdFODMdbcy4WvF3w7P" # PersonalNoPublish; but must have the Publish API key here, in order to publish! Though note: [Cannot see my personal access token in Zenodo · Issue #1586 · zenodo/zenodo · GitHub](https://github.com/zenodo/zenodo/issues/1586) ... (well, it seems it published with this one, too ?!)

# SO:107705; unbuffer for both python 2 and 3; to have this work: python3 papers-put-zenodo-publish-smc2018.py 2>&1 | tee _publish_2018.log
buf_arg = 0
if sys.version_info[0] == 3:
...
# NOTE: just retrieve for the current conference, using 'status': 'draft', and
r = requests.get('https://zenodo.org/api/deposit/depositions',
                  params={'size': 2000,
                          'status': 'draft',
                          'q': 'SMC2018', # this works! returns 77, as it should
...
for ix, item in enumerate( sorted(r.json(), key=lambda k: k['id']) ):
